...
export class Telegraph {
	...
	private async request<T, U = void>(method: Method, payload?: U): Promise<T> {
		...
		const json = (await response.json()) as { ok: any; error: string; result: T };
		if (!json.ok) {
	 ...
	 */
	async createAccount(options: types.CreateAccountOptions): Promise<types.Account> {
		return await this.request<types.Account, types.CreateAccountOptions>("createAccount", options);
	}
	...
	async editAccount(options: types.EditAccountOptions): Promise<types.Account> {
		return await this.request<types.Account, types.EditAccountOptions>("editAccountInfo", options);
	}
	 ...
	 */
	async getAccount(
		fields: types.GetAccountInfoFields = ["short_name", "author_name", "author_url", "auth_url", "page_count"]
	): Promise<types.Account> {
		return await this.request<types.Account, types.GetAccountInfoOptions>("getAccountInfo", {
			fields,
		});
	}
	 ...
	 * account.
	 */
	async revokeAccessToken(save = true): Promise<types.RevokeAccessTokenResponse> {
		const accountCreds = await this.request<types.RevokeAccessTokenResponse>("revokeAccessToken");
	...
	async create(options: types.CreatePageOptions): Promise<types.Page> {
		...
		return await this.request<types.Page, types.CreatePageOptions>("createPage", options);
	}
	 ...
	 * @returns
	 */
	async edit(path: string, options: types.EditPageOptions): Promise<types.Page> {
		if (!options.title) {
	 ...
	 * first.
	 */
	async getPages(options: types.GetPageListOptions = { limit: 50, offset: 0 }): Promise<types.PageList> {
		return await this.request<types.PageList, types.GetPageListOptions>("getPageList", options);
	}
	 ...
	 * ```
	 */
	async getPageViews(path: string, options?: types.GetViewsOptions): Promise<types.PageViews> {
		return await this.request<types.PageViews, types.GetViewsRequestOptions>("getViews", {
			path: path,
			...options,
		});
	}
